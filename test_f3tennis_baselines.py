...
from train_f3tennis_baselines import F3Tennis, evaluate
...
def main(model_dir, frame_dir, split, dataset):
    ...
    classes = load_classes(os.path.join('data', dataset, 'events.txt'))

    model = F3Tennis(len(classes) + 1, config['feature_arch'], config['temporal_arch'], clip_len=config['clip_len'],
